...
class XlsxStreamer {
	...
	public function open($filePath) {
		...
		if (!$this->zip->open($filePath,ZIPARCHIVE::CREATE)) $result = false;
		...
		if ($result) {
			...
			if ($this->sheet == false) {
				...
				if ($this->sheetTemplate == null) $this->sheetTemplate = file_get_contents("{$this->classDirectory}/template/xl/worksheets/sheet1.xml");
